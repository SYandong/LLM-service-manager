...
- #250 TUI 反馈修订：endpoint 复制、GPU 数值列对齐、本地即时过渡、底栏版本与去杂项（v1.1.1；详见 DESIGN §6）
...
| 共享卡外部占用阈值 | #16 | 用 M1 短样本、压力回放与保守配置确定初始值；长期分布后续校准 |
| 内存预算 200 GB、宿主下限 150 GB | #12 | 与内存大户确认 |
| 分支保护 | #2 | 仓库所有者 |
| 连续安静期的可信在途来源；当前 v252 SSE 不满足保证，自动 reload 保持禁用 | #20 / #53 | 按 #53 设计审核确定来源与保证 |
...
| M2 | policy 保护/内存策略、core 意图持久化与动作、client 命令；ops 准备 TTL/reaper/concurrency 切换及回滚 | 保护、dry-run、回放与相应临时环境集成通过后，按 DESIGN §7 做分钟级短测并记录真实范围；不再等待一天或一周。生产动作仍保留相应安全、回滚与操作授权条件。 |
...
| M2 | dry-run 不调用写操作、不持久化意图或租约；pin 经重启/TTL/free 仍受保护且到期有效；默认模型不被策略硬停；普通/pin/默认模型的 sleep 前内存准入及阻塞结果；有起止、样本、错误/缺口和清理证据的短测。 |
...
用户在 #108 明确取消固定一天/一周等待。#8/#16 改用有范围边界的短时采样、回放及保守初始阈值验证；#28 改为处理获准目标后的短时复查与自启动配置核对。短测不证明长期稳定性，未测范围必须记录，但不单独阻塞本轮开发或 alpha 发版。

其余门槛保留在各自 issue：#2 的仓库 admin 操作、#28 相关服务所有者同意、#12 初始内存阈值的相关使用者确认，以及生产动作所需的安全条件、已准备并验证的切换/回滚与最后授权。公开或群内公告尚未获准，#28 的公告验收继续待办。只能在没有其他可推进工作时报告这些实际阻塞，不能把日历等待重新加入，也不能以代码或短测替代未满足的功能与安全验收。
