...
常规热重载仍遵守上述连续 quiet、采用与独立结清证明；不能把维护模式的
...
所有必需布尔事实必须为 true；未知、迟到、错实例或错误配置保持全局屏障。
新实例身份必须不同；监听器消失或命令成功不能单独作为结清证明。
控制中断是显式的，不承诺零中断，也不放宽默认／pin／在途及 RAM 保护。

首次实际维护声明在同一事务中建立 schema6 上下文与 catalog 声明，保存原始
配置、实例和后端身份、动作提交记录及独立观察结果。每个外部动作先持久化
提交标记；超时不能被当作未执行，也不得重发未知 stop/start。实例观察与
副作用共用剩余期限，普通代理退出不释放模型账户。移除目标仍走普通受保护
动作与精确 lease/unit 身份检查，只有正面退出观察才释放对应账户。

显式恢复可以核验已发生的结果、完成目录发布和回执退役，不重放旧动作。
明确回滚必须先确认本次尝试及其 helper 已结清，再恢复原配置并核验新的恢复
实例；不恢复旧账本快照。回滚后的目录使用新的 epoch，不能让旧采集覆盖它。
只有账本明确没有外部动作提交、原配置和自有回执均未变化，才可中止未开始的
尝试；这不是强制清除或伪造同实例转换证明。未知结果继续保留屏障和记账。

正常入口没有隐含站点权限或虚构证明。配置适配器使用固定 argv 和有界关联
...
来源统计使用生产者在**同一条活动记录**中保存的原生 `src`，与该记录的模型、
状态及 token 字段一起读取。完成时间接近、路径和状态相同不能证明请求身份；
不以这些条件关联 journal 来填充来源，不回填历史未知值。原生连接对端也不等于
经过认证的人：代理已丢失原始对端时，不把代理地址映射成原始客户端容器。
仅在相关时间窗口的直接对端与容器映射已验证且未重分配时使用容器标签；
未验证的归属保持未知。现有来源字段和 usage 分组接口足够，不新增公开身份断言。

原生配置 generation reader 显式选择与固定来源版本兼容的 MCP 协议。
既有 v252 的 `path` 请求与完整标量响应解析保持默认行为；需要 `query` 的
来源使用独立、严格的请求和完整响应约定，不自动探测、回退或重试另一种协议。
RPC ID、工具错误、标量、文本包裹、响应大小或期限不符合约定时均不可形成采用证据，
HTTP 200 不豁免这些检查。该选择及其实际调用路径在同一功能 PR 中交付。

可信本地配置将来源版本、构建产物摘要与协议选择关联；实际调用还须证明匹配的是
配置服务的当前运行实例。读取运行中的可执行映像，不只散列可能被替换的路径；
对进程身份及已打开映像的有界读取做前后检查，并在同一剩余期限内对原生查询
前后的实例、映像和配置进行重验。未知版本、来源映射、映像或实例在原生请求前
拒绝；查询期间变化使结果失效。PID 与映像本身不证明目标监听端点归其所有，
实际调用者仍须提供相应的服务/端点所有权证据。抽样前后相同不宣称排除了所有
未观察到的变化再恢复。旧实例、新实例及回滚实例分别绑定，不沿用前一实例的证明。

启用绑定 reader 时，首次维护声明在任何 source 副作用前持久化严格的
`native_provenance`：固定 endpoint、完整 witness settings 和已验证的
`base_generation`。`phase_images` 分别固定 old／candidate／restored 的目标映像摘要；
每个摘要必须对应显式可信来源映射，另一个被允许的映像也不能代替本阶段目标。
设置、来源或 endpoint 在待定事务中变化时拒绝继续，不在旧声明上启用、关闭或
推断新证明。旧版严格解析器拒绝新字段，不能为兼容而删字段或自动迁移未知声明。

绑定模式下，适配器须核实该固定来源与阶段信息，仅提供独立的
`configuration_file_confirmed`、配置摘要、实例以及原有 helper／后端／清理／
尝试结清事实；不额外发送未绑定的旧协议 RPC，也不替 core 宣称 generation 已采用。
core 通过实际映像、实例与监听端点绑定的查询生成采用证据。没有该显式模式的
旧调用保留原协议；无效的绑定模式不能退回旧调用或放宽任一结清条件。

当前严格标量协议不能证明 absent/null generation。未标记的 base 因而在副作用前
拒绝，不能静默写入哨兵或补造回滚可见性。首次可用的 base generation 须通过已审核、
固定摘要的部署／bootstrap 配置阶段明确建立，并由实际 native 查询核验；不能直接
给运行中的配置补标签。固定单一映像的 native profile 也不自动具备可执行文件替换能力：
不同 old／candidate／restored 映像须另有明确实现并核验的安装、启动及回滚路径，
否则该操作只接受实际固定的同一映像。配置声明与测试用映像映射都不是现场切换证明。

来源兼容部署复用经过审核的维护事务及其实际执行路径，并验证当前配置、来源、
备份和回滚后新活动记录的保留；仅安装新产物或看到新 generation 不完成这些验收。
采用可见性不证明连续 quiet、旧请求、helper 或模型资源结清；未确定结果继续
保留事务屏障与记账。部署后只对已验证来源路径的新记录做有界分组对账，
保留历史未知桶及原有端点行为。源码/协议兼容测试不替代现场来源安装或映射证据。

用量表示实际记录到的请求与 token，不保证覆盖全部流量。流式调用需上游返回
usage（客户端可请求 `stream_options.include_usage`）；缺少返回或持久化失败
不能解释成真实零消耗。读取失败仍遵守 #167 的原因与未知值约定，不用缓存成功值
掩盖失败，不据此给出精确计费或长期完整性承诺。
...
- 命令行优先（#168，2026-09-14）：启动即聚焦输入框，任何鼠标点击都不夺走它的焦点；
  Tab 补全、↑/↓ 历史、Shift+↑/↓ 或 Shift+Tab 选中模型、Ctrl+O 打开条目菜单、
  Ctrl+L 清事件显示、Ctrl+C 两次退出、Ctrl+D 空行退出、`?` 仅空行显示帮助。
  界面动作用 `/` 前缀（`/help` `/quit` `/usage` `/events` `/clear` `/copy` `/refresh`），
  不构成第二套命令语义；其余输入仍走 `cli/llm` 的解析器与执行函数。
- 模型/服务操作都经 scheduler API；仅用户明确触发的文本导出和终端剪贴板请求有本地交互副作用，不修改服务配置。

### TUI 交互与渲染约束（#168，2026-09-10）

面向共享服务的终端使用者，首要任务是快速辨认资源、保留选择并安全提交意图。
沿用本节的命令/确认/未知状态契约，不引入第二套 CLI、状态机或后端路由。
视觉使用深灰背景和克制的暖色强调；告警/错误保留文字与语义色，未知值暗显，
不能只靠颜色表达状态。去掉 Header/Footer，在最底部一行显示连接与快捷键；
保留可滚动的结果/详情、键盘焦点和取消优先的 RAM 确认框。

- 模型列宽由终端布局固定，数值右对齐；按模型 key 增量添加/删除/修改单元格。
  不变快照不清表、不重建行，源列表顺序变化不移动当前选择；宽度变化才重排列。
  100×30 为常规基线，60×24/40×24 上下排列，长名称/结果仍可查看。
- GPU 行显示已用/总 GiB 与 llmsvc 记账、外部占用，颜色随占用；缺任一有效量时显示 `?`。
  数值表示总用量，不把规划容量、reservation 或外部占用归因混成实测释放。
- 条目菜单是 overlay 浮层而非模态窗口，英文条目顺序固定，按模型状态置灰但仍显示；
  前四项等价于手输 `preload/wake/sleep/stop MODEL`，`stop` 先在结果区内联 `[y/N]` 确认。
- HTTP 读取在后台，状态轮询默认 0.5 秒（可配），收到 scheduler 事件立即再读一次；
  在途读取不叠加轮询，后台读取结果只写底栏，不覆盖命令结果。SSE 线程只发脏通知，界面合并
  更新至最多 2Hz；无数据时不触发事件重绘。按顺序追加日志，只有明确重置或
  乱序时间戳需要重排有界日志。注释心跳不记事件，连接状态不变不更新组件。
  保留 200 条事件历史和最多 500 行可见日志、重连游标及退出清理保护。
- 进行中的命令显示实际目标、单调时钟耗时与已知阶段。wake 仅使用当前提交后
  同目标的新 wake_requested/place/lease_confirmed/wake_progress，标为“observed”；
  它们没有 request ID，不证明属于本次请求，HTTP 回执才结束本次操作。
...
- 验证采用实际 PTY 的 60 秒只读合成 HTTP 场景：CPU 以单核 100% 定义、目标
  ...
  显示延迟独立，短测不代表长期稳定性。部署由 ops/integration 按当前授权执行。

### 事件可读性与主动导出（#168 用户截图反馈）

事件区默认显示简洁变化，不逐行重复 JSON 和长信任说明。连接状态、在途数
（未知仍为 ?）、已观察错误次数/最近原因及本地丢弃数进入固定状态区。
重连后未变的模型状态、相同错误及相同丢弃类别合并显示；实际状态变化和新的
错误类别仍有事件行，所有原始事件进入原有最多 200 条历史。错误/丢弃计数指
本地游标重置后收到的事件，不是上游完整总量。data-plane 标签明确是原始观测，
不把 observed stopped 当作 daemon stop 或资源释放。定期无变化的 scheduler
state 通知不刷屏，观测错误发生/恢复仍显示。

Details 按钮或非输入焦点下的 e 打开冻结的纯文本详情；保留完整原始 JSON、
source/received_at、unknown、trusted_for_quiet、客户端投递缺口及本地分项计数。
详情包含信任边界说明，主列表保持简短。键盘 Shift+方向键选择经过最低/当前
Textual 验证；不承诺未测试的远程鼠标选择行为。

Copy 只在用户明确操作时请求终端剪贴板；有选区复制选区，无选区复制简洁摘要，
不默认塞入双份完整 JSON。超过 64KiB 明确要求缩小选区或 Save text，不静默
截断。文案为“Copy requested. If your terminal blocks clipboard access, use Save
text.”；不宣称本机/SSH 客户端剪贴板已确认，也不调用远端 OS 剪贴板工具。
Save text 仅由显式按钮把完整冻结详情写入用户选定的新 UTF-8 文件，权限 0600，
拒绝覆盖文件/符号链接。打开、查看、复制、取消均不自动写文件。该路径与
纯文本查看构成无剪贴板支持时的回退；文件保存在运行 llm 的机器，不是假称
...
### TUI 反馈修订（#250，2026-09-15）

基于 v1.1.0 的用户反馈，四项约束保持既有稳定观测、复制映射与 Textual 0.70
兼容：

- **独立的 endpoint 复制**：模型菜单新增「Copy model endpoint address」。复制的是
  共享 llama-swap OpenAI 兼容 base URL，调用方仍使用当前模型名。地址来自独立的
  可选配置 `api_url` / 环境变量 `LLM_API_URL`，经与 scheduler 地址相同的标准库
  http(s) 校验（拒绝凭据、query、fragment、控制字节），根路径规范化为 `/v1`，
  显式非根 base path 原样保留；**不从 scheduler 管理 `url` 或端口推断**。未配置时
  菜单项可见、置灰并直接标注 `needs api_url`（`/help` 也给出设置路径），无需选中
  置灰项即可知道原因，绝不用管理地址冒充推理地址。复制只请求终端剪贴板，不发模型
  请求、不改服务状态、不落盘。省略该配置时旧配置文件与所有 CLI 行为完全不变
  （结果 dict 不新增键）。
- **GPU 数值列对齐**：每卡数值字段（used/total/llmsvc/external/free 与 GPU index
  前缀）在一次渲染内右对齐到共享宽度，0/9/10/99/100/144、分数与未知 `?` 都不会
  让占用条或后续标签错位；两条 bar 仍共用同一实际 `total_gb` 分母並保持 6/10
  宽度，不新增行高，六卡在 40/60 列仍完整可见，缺失/恢复与复制映射不变。
- **本地即时过渡**：点击/提交模型操作时，在入队处同步把目标相写入 STATE 单元
  （`SSDtoMEM` / `SSDtoGPU` / `MEMtoGPU` / `GPUtoMEM`，stop 为 `GPUtoSSD` /
  `MEMtoSSD`），不必等 HTTP 或轮询。它是本会话意图显示：不改 `ModelState`、
  snapshot 或策略输入，不伪造进度。起始态未知时显示诚实的 `loading` / `queued`，
  不臆造 SSD；队列位置仍是模型名后的独立 `[qN]` 标记。本地活动过渡优先于延迟的
  后端过渡，无本地操作时保留后端 `transition`；取消、失败、完成、退出清除对应
  本地 owner，dry-run、解析失败和取消的确认不登记，迟到响应不能复活已结束意图；
  同模型序列先显示在跑目标，随后显示下一个排队意图。活动过渡在派发时按当时已知
  起始态**捕获一次**并贯穿整个执行期：执行中观测变成中间态、目标态或未知都不能
  替换或清空它，也不能落回队列里尚未派发的意图；队列起点在真正派发时用最新状态
  重算。派发时按最新观测重算起始态。
- **底栏版本与去杂项**：底栏固定以运行版本开头（版本来自 CLI 唯一的 argparse
  字面量，不新建发布字面量），40 列长连接/队列提示也不会把它挤掉；移除模型名/
  `source unavailable` 详情行与打开菜单时的 `Menu for …` 提示，归还行高但保留
  真正的错误、确认、命令结果与 `status --json` 诊断。
