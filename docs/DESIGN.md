...
节奏请求**已有 sampler**，不另造采集器；调度延迟不能被当成新证据。用于累计的
发布必须不同，源时间与本地 monotonic 接收时间递增且新鲜（至多 2 秒），相邻有效
观测间隔不超过 2 秒。相关源缺失、未知、过期、乱序、重复、间断或实例变化重置
该判据；不通过删除错误信息把快照伪装成正常。实例仍 active 时必须核对当前
...
stop**，并与最后一次身份/取消检查及普通动作在同一把全局锁下串行化。仅停止
该实例；已经有退出证明则不把不存在的 unit 当新实例操作。只有配置 unit 的正面
