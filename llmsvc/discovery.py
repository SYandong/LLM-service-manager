...
SUPPORTED_KEYS = ("base", "name", "util", "max_model_len", "aliases", "weights_gb",
                  ...
                  "concurrency_limit", "concurrency_queue")
...
class Candidate:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"name": self.name, "path": self.path, "base": self.base,
                ...
                "concurrency_queue": self.overrides.concurrency_queue,
...
def parse_import_config(document: Any, *, directory_name: str) -> tuple[str, str, ImportOverrides]:
    ...
    return name, base, ImportOverrides(util=_optional_util(document.get("util")),
                                       ...
                                       concurrency_queue=_optional_concurrency_queue(document.get("concurrency_queue")))
...
def _optional_concurrency_queue(value: Any) -> int | None:
    if value is None:
        return None
    if type(value) is not int or not 0 <= value <= MAX_CONCURRENCY_QUEUE:
        raise RegistryError("concurrency_queue must be an integer between 0 and %d" % MAX_CONCURRENCY_QUEUE)
    return value
