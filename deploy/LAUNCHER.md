...
When the scheduler rejects placement (HTTP 409 or another failure status on
`/v1/place`, launcher exit 75) outside `--dry-run` and the launcher's parent
process is `vllm-wrapper`, the launcher sends that parent SIGTERM so llama-swap reports
