...
"""Actual #152 HTTP/copy-CLI listing details; the write CLI is gone."""
import hashlib
import json
import shutil
import subprocess
...
def test_copied_cli_keeps_basic_fields_and_actual_inventory_details(api,mounted,tmp_path):
    script=tmp_path/'llm'
    shutil.copyfile(Path(__file__).resolve().parents[1]/'cli'/'llm',script)
    before=mounted.files(),mounted.scheduler.events_since(0)
    done=subprocess.run([sys.executable,'-I','-S',str(script),'--url','http://%s:%s'%mounted.address,
                         'models','--json'],cwd=tmp_path,text=True,capture_output=True,timeout=5)
...
def test_client_reads_real_pending_fifo_projection(api,mounted,monkeypatch):
...
def test_client_preserves_unknowns_protection_and_removed_write(api,mounted,condition):
    if condition=='pin':
    ...
    if condition=='unknown':
        assert row['last_used_at'] is None and 'last use=unknown' in text
...
def test_actual_marker_and_write_405_remain_errors(api,mounted):
