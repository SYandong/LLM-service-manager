...
def test_temporary_inventory_protection(registry):
...
def test_remove_reports_protection_and_never_cleans_units(registry):
    import pytest
    ...
    state[0] = replace(state[0], models=state[0].models + (ModelState('fine', state='sleeping', weights_gb=10),),
                       activity=state[0].activity + (Activity('fine', last_request_at=clock[0], in_flight=0),),
                       pins=(Pin('fine', clock[0]+3600, 'fixture'),))
    units.add('fine')
...
def test_inventory_unknown_activity_does_not_fabricate_runtime(registry):
